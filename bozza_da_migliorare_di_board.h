    ...
    void quarantene_() {
        for (int l = 1; l < dimension_; ++l) {
            for (int c = 1; c < dimension_; ++c) {
                if ((((l == Usa.colonna || l == Usa.colonna * 2)|| (c == Usa.colonna * 2 || c == Usa.colonna))&&(l >= Usa.colonna && l <= Usa.colonna * 2) && (c >= Usa.colonna && c <= Usa.colonna * 2)))
    ...
    void airplane_() {
        ...
        for (int l = 1; l < dimension_; ++l) {
            for (int c = 1; c < dimension_; ++c) {// aggiungere il days minore di novanta
                if (((l < Usa.colonna || l > Usa.colonna * 2) || (c < Usa.colonna || c > Usa.colonna * 2))) {
                    int colonna = (rand() + time(0)) % dimension_;
                    int riga = (rand() + time(0)) % dimension_;
                    if (grid_[l][c].state != Sir::q && grid_[colonna][riga].state != Sir::q
                        && ((riga < Usa.colonna || riga > Usa.colonna * 2) || (colonna < Usa.colonna || colonna > Usa.colonna * 2))) {
                        if (riga == 1) {
